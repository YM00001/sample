...
export default {
    ...
    created: function() {
        this.tag_key = 'eventkey' + Math.floor(Math.random() * 1000000);
    },
    mounted: function() {
        this.fetchCastData();
        ...
        this.$nextTick(() => {
            if (!this.attached) {
                let tags = document.getElementsByClassName(this.tag_key);
                Array.prototype.forEach.call(tags, (item) => {
                    item.addEventListener('click', (event) => {
                        let hash = event.target.dataset.value;
    ...
    methods: {
        showContent: function(str) {
            ...
            str = str.replace(/([#＃][Ａ-Ｚａ-ｚA-Za-z一-鿆0-9０-９ぁ-ヶｦ-ﾟー._-]+)/gm, (p1) => {
                return `<span class="tweet-tag-link ${this.tag_key}" data-value="${p1}">${p1}</span>`;
            });
